...
>
> **HeteroServe（服务面）**：核心是 LMCache 分层缓存。8GB 显存装下 7B-AWQ 后 GPU KV 空间很小，所以我把 KV 分层到 GPU/CPU/SSD。用前缀复用负载验证：固定 5K 前缀 + 变化尾巴，命中缓存的请求 TTFT 下降 X%、命中率 Y%。缓存命中率又回馈给调度器——命中高时主动把请求留在便宜 tier，这是 Online Feedback Loop。
>
